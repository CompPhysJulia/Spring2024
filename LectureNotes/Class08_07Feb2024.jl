...
# ╔═╡ c9d71c9d-fed9-4cd6-9f06-cbac42540b1e
using PlutoTeachingTools, Roots

# ╔═╡ 90807a93-f08b-4f1f-b12d-b4cbfa63e2fd
using PlutoUI, CairoMakie, LaTeXStrings

# ╔═╡ 9164d438-c55b-11ee-3d45-0f5687024a11
html"""
<style>
	main {
		margin: 0 auto;
...
# ╔═╡ a04099cd-a232-4072-9396-6914be3746f0
html"""
<style>
input[type*="range"] {
	width: 70%;
...
# ╔═╡ 1d22c311-7c4e-44ac-be51-1576aa28bef6
TableOfContents()  

# ╔═╡ bdda15c5-335b-4790-94ec-c7df50798144
md"""
# Computational Physics: Class 08 $~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~$ 7 Feb 2024
"""

# ╔═╡ 92cc306e-574a-4676-800f-470e7cd32605
md"## Outline"

# ╔═╡ ceea8f82-9871-4a5d-beeb-a230f66d3d2e
md"""
!!! tip "Topics for today"

	1. Questions on HW4?
	2. Update on using Sliders (Interact.jl and WebIO.jl); best bet is to use Pluto :-)	
	3. Appending elements to an array
	4. for loops vs while loops
	5. The Euler Method for numerical integration
"""

# ╔═╡ 64282da8-535f-41ff-8f99-e0b2eb690cf0
md"""
## Using sliders in Jupyterlab
A recent update to Jupyter seems to have broken something with WebIO.jl and Interact.jl such that sliders are not working in Jupyterlab. 
Hence, at the moment, the easiest way to use sliders is in a Pluto notebook. That being said, one can (with considerably more effort) use sliders in the 
Makie ecosystem. 
"""
...
md"""
!!! todo
	Add link to Makie Slider example
"""

# ╔═╡ f163e985-f59d-48ff-9275-e497b8dd1b2a
md"""
## Appending elements to an array
In julia, if you want to append a value, say b, to some array *x*, you can use either
...
append!(x,b)
```
There are differences between the two functions which you can read about [here](https://stackoverflow.com/questions/72916749/append-vs-push-in-julia) 
and [here](https://www.jlhub.com/julia/manual/en/function/append-exclamation).
...
So far we've primarily used *for* loops. A *for* loop is used when you want to iterate over some known number of steps or over an array of values with a known length. For example, if we want to sum the numbers from 1 to N, we can write
...
However, suppose you have a situation where you don't know ahead of time how many steps a process will take. Then, you want to use a *while* loop.
A *while* loop will have a logical condition which, if true, will continue to execute until the condition is false. For example, suppose you release a ball from some height $h$ and it falls under the influence of gravity. If you can ignore air resistance, then the height of the ball as a function of time is given analytically as 

$y = h - \frac{1}{2}g t^2$

and if you wanted to make a plot of the $y$ position as a function of time but only while $y \ge 0$, you could write a function to compute the needed 
...
md"""
!!! note
    Try out the while loop in the cell below, and then plot the result using either Plots.jl or **CairoMakie.jl**.
"""
...
md"""
!!! hint
	Don't forget to make the plot you have to either import Plots.jl or CairoMakie.jl. You'll also want to label your axes.
"""
...
# ╔═╡ 6abcbb07-9e9f-4957-b995-2e79bdd5dfd5
md"""
### Theoretical solution
Instead of using a while loop as we did previously, I will use the Roots.jl package to find out when the object reaches the ground. 
Here's a function to compute the time, position, and velocity arrays:
"""

# ╔═╡ 860ec8b2-5d60-4fe9-b7e1-41b4f086a4f1
begin
...
md"""
### Euler method soltution
The cell below implements the Euler method to numerically solve for 
...
manifest_format = "2.0"
project_hash = "549e464bfc2f46ef9af0c9ee3e63a7d5539d6f30"
