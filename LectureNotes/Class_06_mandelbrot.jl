...
#> title = "Mandelbrot"
#> date = "2024-01-30"
#> tags = ["fractal", " mandelbrot", "CairoMakie"]
#> description = "Simple interactive Mandelbrot set plotting in Pluto.jl"
#> 
#>     [[frontmatter.author]]
#>     name = "Paul Nakroshis"
#>     url = "http://portlandphysics.me"
...
# ╔═╡ 49f66492-8d28-462f-ac82-2d53c3ae930e
using PlutoUI

# ╔═╡ 00f294aa-4aba-4b17-8b2c-4a21c6e6f1a7
using CairoMakie

# ╔═╡ ca5e8db2-bf87-11ee-37d5-55cba2af467f
html"""
<style>
	main {
		margin: 0 auto;
...
# ╔═╡ 93546ec4-8196-4584-8905-cd3fa5ec40ec
md"""
# The Mandelbrot Set
The mandelbrot set was first crudely visualized in 1978 by Robert W. Brooks and Peter Matelski:
"""

# ╔═╡ 68e4aa4d-4c1f-43fe-be10-f39c69ab8e84
Resource(
	"https://upload.wikimedia.org/wikipedia/commons/d/d7/Mandel.png",
	:alt => "Mandelbrot set first plot",
	:width => 400
)

# ╔═╡ c62c13e5-d252-46f3-a310-a344d94db36c
md"""
A few years later, Benoit Mandelbrot, working at the IBM Watson Research center, made the first high resolution plot, and 
the mandelbrot set became named after him. Today, we'll see how this remarkable set is defined, and how to visualize it using 
CairoMakie.
"""

# ╔═╡ 5a4d453b-914b-47fb-a861-f6f4990fc027
md"""
## Mathematical background: Complex numbers in Julia

A complex number is composed of a real and an imaginary component and is graphically represented by a real axis (the x-axis)
and the imaginary axis (the y-axis). We can write a complex number $z$ as 

$$z = x + i y,$$

where $x$ is the real component, $y$ is the imaginary component, and $i = \sqrt{-1}$.

In Julia, the ability to use and manipulate complex numbers is built-in. 
For example, we can create a complex number $z = 3 + 4i$ by typing
...
Some common computations with complex numbers can be seen in the [Julia documentation](https://docs.julialang.org/en/v1/manual/complex-and-rational-numbers/).

"""

# ╔═╡ 2785d5c8-38e4-4314-8be1-497e858f5e58
md"""
# Defining the Mandelbrot set
The mandelbrot set is the set of points in the complex $c$ plane that do not let the origin in the complex z-plane diverge to infinity under the 
iterative process

$$z \rightarrow z^2 + c,$$

where the iteration starts at $z=0$. In practice, we know that once the magnitude of $z$ exceeds 2, successive iterates will diverge to $\infty$. It's faster to compute the absolute square of a complex number, so we'll check at each stage of iteration whether $z^2 \ge 4$. For each point in the c-plane, we record how many iterations, Nmax, it takes to escape our threshold; if a point does not escape our threshold after Nmax iterations, we provisionally say it is in the mandelberot set. The plot is made by coloring the points based on how long it took to exceed our threshold. The available colormaps can be seen on the [Makie website](https://docs.makie.org/stable/explanations/colors/).

Here's a function to implement this:
"""

# ╔═╡ d321e1b0-ca83-4adc-8ef4-a2790c98a407
begin
	function mandelbrot(x, y, Nmax)
    	c = complex(x, y)
    	z = 0.0 + 0.0im
    	for i in 1:Nmax
        	z = z^2 + c
...
# ╔═╡ 2d4c0f5b-6b9e-4b82-97cb-e8b0b0cd5548
md"""
Now we'll use some of the wonderful features of Pluto notebooks to create an interactive
interface to visualize the Mandelbrot set.
"""

# ╔═╡ 2b46ec21-b8d5-4685-ad7a-2d97004f5be3
@bind x_min NumberField(-3:0.001:1, default=-2.0) 

# ╔═╡ f0388dd7-083c-4f28-b2d5-6e4a4c62704d
@bind y_min NumberField(-1.5:0.001:1.5, default=-1.0)

# ╔═╡ d879f0b1-69c0-45ce-bbd5-b2f2cb2c9c10
@bind x_max NumberField(-2.9:0.001:1.1, default=0.5)

# ╔═╡ 7babcc1c-e2da-43f4-95a6-442ca2d0470c
@bind y_max NumberField(-1.4:0.001:1.6, default=1.0)

# ╔═╡ 69060798-fc20-40b1-86eb-aa08bba2d624
dx = 0.00005

# ╔═╡ 19c2f63c-6491-4499-97c5-e84e8a940bd4
@bind Nmax NumberField(10:1000, default=100)

# ╔═╡ d71529ac-4465-4d4e-a939-ae6f049ef605
let
		x = x_min:dx:x_max
		y = y_min:dx:y_max
		z = [mandelbrot(x, y, Nmax) for x in x, y in y]
		Nx = ceil(Int,(x_max - x_min)/dx)
		Ny = ceil(Int,(y_max - y_min)/dx)
		fig = Figure(size = (Nx, Ny))
		ax = Axis(fig[1, 1])
		hm = heatmap!(ax, x, y, z, colormap = :RdGy_11)
		fig
end
